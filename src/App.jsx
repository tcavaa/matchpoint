...
const APPS_SCRIPT_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbxEI9gdf7kWAiLMlhxABmGQPosESnD5iI6VCPwLVdD4ICbXs-ToHuucNNj8cSIS0_a7YQ/exec';
  ...
  if (storedTables) {
    try {
      ...
      return parsedTables.map(table => ({
        ...
        timerStartTime: table.isRunning && table.timerStartTime ? table.timerStartTime : null,
        elapsedTimeInSeconds: typeof table.elapsedTimeInSeconds === 'number' ? table.elapsedTimeInSeconds : 0,
        isRunning: typeof table.isRunning === 'boolean' ? table.isRunning : false,
        timerMode: table.timerMode || 'standard',
        initialCountdownSeconds: typeof table.initialCountdownSeconds === 'number' ? table.initialCountdownSeconds : null,
      }));
    } catch (e) { console.error("Error parsing stored tables in initializeTables:", e); }
  }
  return Array.from({ length: 10 }, (_, i) => ({
    id: i + 1, name: `Table ${i + 1}`, timerStartTime: null, elapsedTimeInSeconds: 0,
    isRunning: false, timerMode: 'standard', initialCountdownSeconds: null,
  }));
};

const initializeHistory = () => {
  const storedHistory = localStorage.getItem(LOCAL_STORAGE_HISTORY_KEY);
  console.log("InitializeHistory: Loading from localStorage. Key:", LOCAL_STORAGE_HISTORY_KEY, "Data:", storedHistory);
  if (storedHistory) {
    try {
...
function App() {
        ...
        if (table.isRunning) {
          ...
          if (table.timerMode === 'countdown' && table.initialCountdownSeconds && table.timerStartTime) {
            ...
            if (totalPassedTime >= table.initialCountdownSeconds) {
              ...
              console.log(`Table ${table.name} countdown finished automatically.`);
  ...
  useEffect(() => {
    console.log("SessionHistory Effect: Attempting to save history. Current state:", sessionHistory);
    try {
      localStorage.setItem(LOCAL_STORAGE_HISTORY_KEY, JSON.stringify(sessionHistory));
      console.log("SessionHistory Effect: Successfully saved to localStorage. Key:", LOCAL_STORAGE_HISTORY_KEY);
    } catch (e) {
  ...
  const handleStartTimer = useCallback((tableId, mode, durationMinutes) => {
    setTables(prevTables =>
      prevTables.map(table => {
        if (table.id === tableId) {
          if (mode === 'countdown' && durationMinutes && durationMinutes > 0) {
            return {
              ...table,
              timerMode: 'countdown',
    ...
    setTables(prevTables =>
      prevTables.map(table => {
        if (table.id === tableId && table.isRunning && table.timerStartTime) {
          const elapsedSinceLastStart = (Date.now() - table.timerStartTime) / 1000;
          return {
            ...table,
            isRunning: false,
            elapsedTimeInSeconds: table.elapsedTimeInSeconds + elapsedSinceLastStart,
            timerStartTime: null,
          };
        }
        return table;
  ...
  const handlePayAndClear = useCallback((tableId) => {
    ...
    if (tableToClear.isRunning && tableToClear.timerStartTime) {
      finalElapsedTimeInSeconds += (Date.now() - tableToClear.timerStartTime) / 1000;
    }
    ...
    const amountToPay = parseFloat(calculateCost(durationForBilling, HOURLY_RATE));
    ...
    console.log("handlePayAndClear: Created newSessionDetails:", newSessionDetails);
    ...
    setTables(prevTables =>
      prevTables.map(table => {
        if (table.id === tableId) {
          ...
          return {
            ...table,
            timerStartTime: null, elapsedTimeInSeconds: 0, isRunning: false,
            timerMode: 'standard', initialCountdownSeconds: null,
          };
        }
        return table;
      })
    ...
    console.log("handlePayAndClear: newSessionDetails is valid, attempting to update local sessionHistory.");
    setSessionHistory(prevHistory => {
      const updatedHistory = [...prevHistory, newSessionDetails];
      console.log("handlePayAndClear: New local history to be set:", updatedHistory);
      return updatedHistory;
    });

    // 👇 --- NEW: Send data to Google Sheets --- 👇
    if (APPS_SCRIPT_WEB_APP_URL === 'YOUR_COPIED_WEB_APP_URL_HERE' || !APPS_SCRIPT_WEB_APP_URL) {
      console.warn("Google Sheets Sync: APPS_SCRIPT_WEB_APP_URL is not set. Skipping sync.");
    } else {
      console.log("Google Sheets Sync: Sending data to Apps Script:", newSessionDetails);
...
.then(({ ok, status, data }) => {
  if (ok && data.status === 'success') {
    ...
    console.error('Google Sheets Sync Error (from script or network):', data);
    // Log more details if available from the 'data' object from your script
    if(data && data.message) console.error('Script error message:', data.message);
  }
})
.catch(error => {
  ...
  }, [tables, HOURLY_RATE, calculateCost, playSound, setTables, setSessionHistory]);
